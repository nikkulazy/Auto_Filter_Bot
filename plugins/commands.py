...
async def start(client, message):
    ...
    if data.startswith("allfiles"):
        try:
            ...
            for file in files:
                ...
                filesarr.append(msg)
            k = await client.send_message(chat_id=message.from_user.id, text=f"<b><u>⚠️⚠️⚠️IMPORTANT⚠️⚠️⚠️</u></b>\n\nTʜɪs ᴍᴏᴠɪᴇ/ᴠɪᴅᴇᴏ ᴡɪʟʟ ʙᴇ ᴅᴇʟᴇᴛᴇᴅ ɪɴ <b><u><code>{get_time(DELETE_TIME)}</code></u> 🫡 <i></b>(ᴅᴜᴇ ᴛᴏ ᴄᴏᴘʏʀɪɢʜᴛ ɪssᴜᴇ)</i>.\n\n<b><i>Pʟᴇᴀsᴇ Fᴏʀᴡᴀʀᴅ ᴛʜɪs ᴍᴏᴠɪᴇ/ᴠɪᴅᴇᴏ ᴛᴏ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs ᴀɴᴅ sᴛᴀʀᴛ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ʜᴇʀᴇ</i></b>")
            await asyncio.sleep(DELETE_TIME)
            for x in filesarr:
                await x.delete()
            await k.edit_text("<b>Yᴏᴜʀ ᴀʟʟ ᴍᴏᴠɪᴇs/ᴠɪᴅᴇᴏs ᴀʀᴇ ᴅᴇʟᴇᴛᴇᴅ sᴜᴄᴄᴇssғᴜʟʟʏ !\nKɪɴᴅʟʏ ʀᴇǫᴜᴇsᴛ ᴀɢᴀɪɴ</b>")
            return
        except Exception as e:
            logger.exception(e)
            return
    ...
    try:
        ...
        k = await msg.reply(
            ...
            "<b><i>ᴘʟᴇᴀsᴇ ꜰᴏʀᴡᴀʀᴅ ᴛʜɪs ꜰɪʟᴇ ᴛᴏ sᴀᴠᴇᴅ ᴍᴇssᴀɢᴇs ᴀɴᴅ sᴛᴀʀᴛ ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ʜᴇʀᴇ</i></b>",
            quote=True
        )     
        await asyncio.sleep(DELETE_TIME)
        await msg.delete()
