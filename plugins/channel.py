...
# ============================================================
# ✅ MODIFIED: _process_with_lock with Quality Upgrade
# ============================================================
async def _process_with_lock(bot, filename, caption, media_info, base_name, processed, media):
    ...
    # ============================================================
    # ✅ QUALITY UPGRADE CHECK - NEW CODE ADDED
    # ============================================================
    if movie_doc:
        if is_good_quality(filename):
            movie_base_name = get_movie_base_name(filename)
            if movie_base_name:
                try:
                    ...
                    if delete_result["deleted"] > 0:
                        logger.info(
                            f"✅ [CHANNEL] Quality upgraded for '{movie_base_name}': "
                            f"Deleted {delete_result['deleted']} CAM/Theatre files"
                        )
                        # Movie message refresh karein
                        await schedule_update(bot, movie_base_name, delay=2)
                except Exception as e:
                    logger.error(f"Quality upgrade failed for {movie_base_name}: {e}")
