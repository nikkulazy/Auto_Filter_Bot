...
async def advantage_spoll_choker(bot, query):
    ...
    try:
        # Try TMDB first if enabled, else fallback to IMDb
        if TMDB_ON_SEARCH:
        ...
        if not movies:
            await query.answer("No movie details found!", show_alert=True)
            return
        ...
        logger.error(f"Error in spol callback: {e}")
...
# ============================================================
# ✅ SPELL CHECKER FUNCTIONS (FIXED)
# ============================================================

async def ai_spell_check(chat_id, wrong_name):
    """Check spelling and suggest correct movie name using IMDb"""
    try:
        # imdbkit ka search_movie() directly list return karta hai
        search_results = imdb.search_movie(wrong_name)
        
        # Agar list empty hai toh return
        if not search_results:
            return None
        
        # imdbkit search results mein movie objects hain, unka title nikaalo
        movie_list = []
        ...
        # RapidFuzz se best match find karo
        closest_match = process.extractOne(wrong_name, movie_list)
        
        if not closest_match:
            return None
        
        best_match, score = closest_match
        
        # Agar confidence 80+ hai toh check karo files exist karti hain ya nahi
        if score >= 80:
            files, _, _ = await get_search_results(chat_id=chat_id, query=best_match)
            if files:
                return best_match
            else:
                # Clean version try karo (year, quality hata kar)
                clean_name = re.sub(r'\s*\(?\d{4}\)?\s*', '', best_match).strip()
                clean_name = re.sub(r'\b(720p|1080p|4k|bluray|webrip|dvdrip|hindi|english|tamil|telugu|malayalam)\b', '', clean_name, flags=re.IGNORECASE).strip()
                if clean_name and clean_name != best_match:
                    files, _, _ = await get_search_results(chat_id=chat_id, query=clean_name)
                    if files:
                        return clean_name
        
        # Agar exact match nahi mila toh list se koi bhi movie suggest karo
        for movie_title in movie_list[:5]:
            files, _, _ = await get_search_results(chat_id=chat_id, query=movie_title)
            if files:
                return movie_title
        
        return None
    except Exception as e:
        logging.error(f"AI Spell Check Error: {e}")
        return None

async def advantage_spell_chok(client, message):
    """Handle spell check suggestions when no results found"""
    search = message.text
    query = re.sub(
    ...
    try:
        ...
        if movies:
            ...
            # Movies ko buttons mein convert karo
            for idx, movie in enumerate(movies[:5]):  # Max 5 suggestions
                try:
                    # Handle both imdbkit objects and dicts
            ...
            buttons.append([
                InlineKeyboardButton("🔍 Search on Google 🔍", url=f"https://www.google.com/search?q={google}")
            ])
            buttons.append([
        ...
        # Agar IMDb se koi suggestion nahi mila toh spell checker try karo
        settings = await get_settings(message.chat.id)
        if settings.get("spell_check", True):
            corrected = await ai_spell_check(message.chat.id, query)
            if corrected:
                button = [[
                    InlineKeyboardButton(f"📝 Did you mean: {corrected}?", 
                                       callback_data=f"spell_correct#{corrected}")
                ]]
                d = await message.reply_text(
                    text=f"🔍 No results found for <b>{search}</b>\n\n" +
                         f"📝 Did you mean: <b>{corrected}</b>?",
        ...
        google = quote_plus(search)
        button = [[
            InlineKeyboardButton("🔍 Check spelling on Google 🔍", 
        ...
        google = quote_plus(search)
        button = [[
            InlineKeyboardButton("🔍 Search on Google 🔍", 
                               url=f"https://www.google.com/search?q={google}")
        ]]
        k = await message.reply_text(
            text=script.I_CUDNT.format(search),
...
# ============================================================
# ✅ SPELL CORRECT CALLBACK HANDLER
# ============================================================

@Client.on_callback_query(filters.regex(r"^spell_correct#"))
async def spell_correct_callback(client, query):
    """Handle spell correction click"""
    try:
        _, corrected = query.data.split("#")
        user_id = query.from_user.id
        
        # Check if this is the same user who requested
        ...
        await query.answer(f"Searching for: {corrected}")
        ...
        # Create a new message with the corrected search
        class FakeMessage:
        ...
        fake_msg = FakeMessage(query.message.chat.id, query.from_user, corrected)
        await auto_filter(client, fake_msg)
    except Exception as e:
        logging.error(f"Spell correct callback error: {e}")
